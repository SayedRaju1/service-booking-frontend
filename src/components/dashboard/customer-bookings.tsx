...
export function CustomerBookings() {
  ...
  // Fetch all business bookings for statistics (unfiltered)
  const { data: allBookingsData, isLoading: isLoadingStats } = useQuery({
    queryKey: ["businessBookingsStats"],
    queryFn: () =>
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["businessBookingsStats"] });
  ...
  const {
    ...
    queryKey: ["businessBookings", debouncedFilters],
    queryFn: () =>
      bookingsApi.getBusinessBookings({
        status:
    ...
    enabled: debouncedFilters === filters, // Only run when debounced filters match current filters
  });
  ...
  // Calculate statistics from unfiltered data (always accurate totals)
  const totalBookings = allBookingsData?.data?.pagination?.total || 0;
