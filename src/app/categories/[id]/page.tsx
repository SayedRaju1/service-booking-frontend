...
export default function CategoryDetailPage() {
  ...
  const { data: categoryData, isLoading: categoryLoading } = useQuery({
    ...
    queryFn: () => serviceCategoriesApi.getCategoryById(categoryId),
    enabled: !!categoryId,
  });

  const { data: categoryServicesData, isLoading: servicesLoading } = useQuery({
    queryKey: ["category-services", categoryId],
    queryFn: () => serviceCategoriesApi.getCategoryServices(categoryId),
    enabled: !!categoryId,
  });
                ...
                {category.subcategories && category.subcategories.length > 0 ? (
                  <div className="space-y-2">
                    {category.subcategories.map((subcategory) => (
                      <Link
                        key={subcategory._id}
                        href={`/services?category=${subcategory._id}`}
